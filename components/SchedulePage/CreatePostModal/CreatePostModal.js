...
export default function CreatePostModal({ isOpen, onClose, scheduleDate, onPostScheduled }) {
    ...
    const handleGeneratePreview = async (formPayload) => {
        ...
        setMessage('');
        setFormData(formPayload); // Store the form data for the final schedule step
    ...
    return (
        <div className={styles.overlay} onClick={onClose}>
            <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
                ...
                {view === 'CONFIG' && (
                    <>
                        <h2 className={styles.title}>Schedule New Post</h2>
                        ...
                        <h3 className={styles.formTitle}>Choose Post Type</h3>
                        ...
                        <button className={styles.nextButton} onClick={handleNextToForm} disabled={!selectedPostType}>Next</button>
                    </>
                )}

                {view === 'FORM' && (
                    <>
                        <button className={styles.backButton} onClick={() => setView('CONFIG')}>&larr; Back to Config</button>
                ...
                {view === 'PREVIEW' && (
                    <>
                        <button className={styles.backButton} onClick={() => setView('FORM')}>&larr; Back to Edit Details</button>
                        <h3 className={styles.formTitle}>Confirm & Schedule</h3>
                        <div className={styles.previewLayout}>
                            <div className={styles.previewImageWrapper}>
                                {isSubmitting && <div className={styles.imageOverlay}>Scheduling...</div>}
                                <img src={previewUrl} alt="Generated Preview" className={styles.previewImage} />
                            </div>
                            <div className={styles.confirmDetails}>
                                <p><strong>Scheduling for:</strong></p>
                                <p>{new Date(selectedDate).toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })} at {selectedTime}</p>
