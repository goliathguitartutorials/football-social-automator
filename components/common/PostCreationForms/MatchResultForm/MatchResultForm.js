...
// Autocomplete sub-component for Glannau player names
...
// Sub-component for a single Glannau scorer input row
const GlannauScorerInput = ({ scorer, onUpdate, onRemove, players }) => {
    return (
        <div className={styles.scorerRow}>
            ...
            <div className={styles.scorerMinute}>
                <input type="number" placeholder="Min" value={scorer.minute} onChange={(e) => onUpdate(scorer.id, 'minute', e.target.value)} className={styles.minuteInput} />
            </div>
            <div className={styles.scorerPenalty}>
                <input type="checkbox" id={`penalty-${scorer.id}`} checked={scorer.isPenalty} onChange={(e) => onUpdate(scorer.id, 'isPenalty', e.target.checked)} />
                <label htmlFor={`penalty-${scorer.id}`}>P</label>
            </div>
            <button type="button" onClick={() => onRemove(scorer.id)} className={styles.removeScorerBtn}><RemoveIcon /></button>
        </div>
    );
};

// Sub-component for a single Opposition scorer input row
const OppositionScorerInput = ({ scorer, onUpdate, onRemove }) => {
    return (
        <div className={styles.scorerRow}>
            <div className={styles.scorerPlayer}>
                <input
                    ...
                    placeholder="Player Name or 'OG'"
                    value={scorer.name}
                    onChange={(e) => onUpdate(scorer.id, 'name', e.target.value)}
                />
            </div>
            <div className={styles.scorerMinute}>
                <input type="number" placeholder="Min" value={scorer.minute} onChange={(e) => onUpdate(scorer.id, 'minute', e.target.value)} className={styles.minuteInput} />
            ...
            <div className={styles.scorerPenalty}>
                <input type="checkbox" id={`penalty-opp-${scorer.id}`} checked={scorer.isPenalty} onChange={(e) => onUpdate(scorer.id, 'isPenalty', e.target.checked)} />
                <label htmlFor={`penalty-opp-${scorer.id}`}>P</label>
            </div>
            <button type="button" onClick={() => onRemove(scorer.id)} className={styles.removeScorerBtn}><RemoveIcon /></button>
        </div>
    );
...
export const formatScorersForWebhook = ({ glannauScorers, oppositionScorers, isGlannauHome }) => {
    const formatScorerList = (scorers, isGlannauList) => {
        const grouped = {};
        scorers.forEach(s => {
            if (!s.name || !s.minute) return;
            const key = s.name.trim().toUpperCase() === 'OG' ? 'OG' : s.name.trim();
            if (!grouped[key]) grouped[key] = [];
            grouped[key].push({ minute: s.minute, isPenalty: s.isPenalty });
        });

        return Object.entries(grouped).map(([name, goals]) => {
            goals.sort((a, b) => parseInt(a.minute) - parseInt(b.minute));
            const goalStrings = goals.map(g => `${g.minute}'${g.isPenalty ? ' (P)' : ''}`).join(', ');
            if (name === 'OG') return `OG ${goalStrings}`;
            
            if (isGlannauList) {
                const nameParts = name.split(' ');
                const formattedName = nameParts.length > 1 ? `${nameParts[0].charAt(0).toUpperCase()}. ${nameParts.slice(1).join(' ')}` : name;
                return `${formattedName} ${goalStrings}`;
            }
            return `${name} ${goalStrings}`; // No name formatting for opposition
        });
    };

    const glannauLines = formatScorerList(glannauScorers, true);
    const oppositionLines = formatScorerList(oppositionScorers, false);

    const homeScorers = isGlannauHome ? glannauLines : oppositionLines;
    const awayScorers = isGlannauHome ? oppositionLines : glannauLines;
...
export default function MatchResultForm({ appData = {}, initialData, onSubmit, onYoloSubmit, isSubmitting, authKey }) {
    ...
    // --- Glannau Scorer Handlers ---
    const handleAddScorer = () => setFormData(prev => ({ ...prev, scorers: [...prev.scorers, { id: Date.now(), name: '', minute: '', isPenalty: false }] }));
    const handleRemoveScorer = (id) => setFormData(prev => ({ ...prev, scorers: prev.scorers.filter(s => s.id !== id) }));
    const handleScorerUpdate = (id, field, value) => setFormData(prev => ({ ...prev, scorers: prev.scorers.map(s => s.id === id ? { ...s, [field]: value } : s) }));

    // --- Opposition Scorer Handlers ---
    const handleAddOppositionScorer = () => setFormData(prev => ({ ...prev, oppositionScorers: [...prev.oppositionScorers, { id: Date.now(), name: '', minute: '', isPenalty: false }] }));
    ...
    const handleSelectGalleryBg = (bgLink) => setFormData(prev => ({ ...prev, selectedBackground: bgLink }));
    const handleCropComplete = (dataUrl) => setFormData(prev => ({ ...prev, selectedBackground: dataUrl }));
    ...
    const handleGenerateCaption = async () => {
        ...
        const gameInfo = {
            ...
            glannauScorers: formData.scorers.filter(s => s.name && s.minute),
            oppositionScorers: formData.oppositionScorers.filter(s => s.name && s.minute)
        };
        const payload = { page: 'result', gameInfo };
        try {
    ...
    return (
        <form className={styles.formContainer} onSubmit={(e) => { e.preventDefault(); onSubmit(formData); }}>
            ...
            <div className={styles.section}>
                <div className={styles.sectionHeader}><h3 className={styles.sectionTitle}>Background</h3><div className={styles.backgroundTabs}><button type="button" className={`${styles.tabButton} ${backgroundSource === 'gallery' ? styles.active : ''}`} onClick={() => setBackgroundSource('gallery')}><span className={styles.tabIcon}><GalleryIcon /></span><span className={styles.tabLabel}>Gallery</span></button><button type="button" className={`${styles.tabButton} ${backgroundSource === 'custom' ? styles.active : ''}`} onClick={() => setBackgroundSource('custom')}><span className={styles.tabIcon}><UploadIcon /></span><span className={styles.tabLabel}>Custom</span></button></div></div>
                {backgroundSource === 'gallery' && (<div className={styles.backgroundGrid}>{backgrounds.map((bg) => (<div key={bg.Link} className={`${styles.backgroundItem} ${formData.selectedBackground === bg.Link ? styles.selected : ''}`} onClick={() => handleSelectGalleryBg(bg.Link)}><img src={bg.Link} alt={bg.Name} /></div>))}</div>)}
                {backgroundSource === 'custom' && (<div className={styles.customBackgroundContainer}><ImageEditor onCropComplete={handleCropComplete} /><div className={styles.checkboxContainer}><input type="checkbox" id="saveCustomBackground" checked={formData.saveCustomBackground} onChange={handleChange} /><label htmlFor="saveCustomBackground">Save background for future use</label></div></div>)}
            </div>
            <div className={styles.section}>
                ...
                <div className={styles.aiButtonContainer}><button type="button" className={styles.aiButton} onClick={handleGenerateCaption} disabled={isGeneratingCaption}><GenerateIcon />{isGeneratingCaption ? 'Generating...' : 'Generate with AI'}</button></div>
            </div>
            ...
            <div className={styles.actionsContainer}>
                ...
                <div className={styles.yoloAction}><button type="button" onClick={() => onYoloSubmit(formData)} disabled={isSubmitting} className={styles.yoloButton}>{isSubmitting ? 'Sending...' : 'Post Now (YOLO)'}</button></div>
            </div>
        </form>
    );
}
