...
export default function LiveTab() {
    ...
    const reconstructStateFromEvents = useCallback((eventList) => {
        ...
        const normalizedEvents = eventList.map(event => ({
            ...
            playerFullName: event.plaerFullName || event.playerFullName,
        }));
    ...
    const calculateElapsedTime = useCallback(() => {
        ...
        } else { totalSeconds = (now - matchStartTime) / 1000; }
    ...
    useEffect(() => {
        const interval = setInterval(() => { const { display } = calculateElapsedTime(); setElapsedTimeDisplay(display); }, 1000);
        return () => clearInterval(interval);
    }, [calculateElapsedTime]);

    const handleEventClick = (eventType) => {
        const { minute } = calculateElapsedTime(); setPrepopulatedMinute(minute); setSelectedEventType(eventType); setView('logEvent'); setApiError('');
    };
    
    const handleControlClick = async (eventType) => { await handleEventSubmit({ eventType, minute: calculateElapsedTime().minute }); };
    const handleFormCancel = () => { setView('dashboard'); setSelectedEventType(null); };
    ...
    const handleEventSubmit = async (eventData) => {
        ...
        const apiPayload = { eventId, matchId: liveMatch.matchId, eventType: eventData.eventType, minute: eventData.minute, timestamp: eventTimestamp.toISOString(), team: eventData.team || '', playerFullName: eventData.scorer || eventData.player || eventData.playerOff || '', assistByFullName: eventData.assist || eventData.playerOn || '' };
        try {
            const response = await fetch('/api/manage-match', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authKey}` }, body: JSON.stringify({ action: 'log_event', matchData: apiPayload }) });
            if (!response.ok) throw new Error((await response.json()).error || 'Failed to log event.');
            const refetchResponse = await fetch('/api/manage-match', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authKey}` }, body: JSON.stringify({ action: 'get_match_events', matchData: { matchId: liveMatch.matchId } }) });
            ...
            handleFormCancel();
        } catch (error) { setApiError(error.message); } finally { setIsSubmitting(false); }
    };
    
    useEffect(() => {
        if (!matchStartTime || events.some(e => e.eventType === 'MATCH_END' || e.eventType === 'AUTO_MATCH_END')) return;
        const autoEndMinute = 105;
        const checkAutoEnd = () => { if (events.some(e => e.eventType === 'MATCH_END' || e.eventType === 'AUTO_MATCH_END')) return; const { minute } = calculateElapsedTime(); if (minute > autoEndMinute) { handleControlClick('AUTO_MATCH_END'); } };
        const msUntilCheck = ( (autoEndMinute + 1) * 60 * 1000) - (new Date() - matchStartTime);
        if (msUntilCheck < 0) { checkAutoEnd(); return; }
    ...
    if (liveMatch) {
        ...
        return (
            <div className={styles.dashboardContainer}>
                ...
                <div className={styles.matchHeader}><span className={styles.teamName}>{liveMatch.homeTeamName}</span><div className={styles.scoreContainer}><span className={styles.score}>{score.home} - {score.away}</span><span className={styles.elapsedTime}>{elapsedTimeDisplay}</span></div><span className={styles.teamName}>{liveMatch.awayTeamName}</span></div>
                <div className={styles.controlsSection}>
                    {!hasEnded && (<div className={styles.matchControls}>{!hasStarted && <button className={styles.controlButton} onClick={() => handleEventClick('MATCH_START')}><PlayIcon/>Start Match</button>}{hasStarted && !isHalfTime && <button className={styles.controlButton} onClick={() => handleControlClick('HALF_TIME')}><PauseIcon/>Half-Time</button>}{isHalfTime && !hasSecondHalfStarted && <button className={styles.controlButton} onClick={() => handleControlClick('SECOND_HALF_START')}><PlayIcon/>Start 2nd Half</button>}{hasStarted && <button className={styles.controlButton} onClick={() => handleControlClick('MATCH_END')}><StopIcon/>Finish Match</button>}</div>)}
                    <div className={styles.eventGrid}><button className={styles.eventButton} onClick={() => handleEventClick('Goal')} disabled={!hasStarted || hasEnded}><GoalIcon /><span>Goal</span></button><button className={styles.eventButton} onClick={() => handleEventClick('Yellow Card')} disabled={!hasStarted || hasEnded}><YellowCardIcon /><span>Yellow Card</span></button><button className={styles.eventButton} onClick={() => handleEventClick('Red Card')} disabled={!hasStarted || hasEnded}><RedCardIcon /><span>Red Card</span></button><button className={styles.eventButton} onClick={() => handleEventClick('Substitution')} disabled={!hasStarted || hasEnded}><SubIcon /><span>Substitution</span></button></div>
                </div>
                <div className={styles.detailsContainer}>
                    <div className={styles.tabBar}><button className={`${styles.tabButton} ${activeTab === 'overview' ? styles.active : ''}`} onClick={() => setActiveTab('overview')}><OverviewIcon /><span>Overview</span></button><button className={`${styles.tabButton} ${activeTab === 'squad' ? styles.active : ''}`} onClick={() => setActiveTab('squad')}><SquadIcon /><span>Squad</span></button></div>
                    <div className={styles.panelContainer}>{activeTab === 'overview' && <MatchEventsPanel events={events} match={liveMatch} />}{activeTab === 'squad' && <SquadPanel squadList={liveMatch.squadList} />}</div>
                </div>
            </div>
        );
    }
    if (nextMatch) {
        ...
        return (<div className={styles.placeholder}><h3>Next Match</h3><p>{nextMatch.homeOrAway === 'Home' ? 'CPD Y Glannau' : nextMatch.opponent} vs {nextMatch.homeOrAway === 'Away' ? 'CPD Y Glannau' : nextMatch.opponent}</p><CountdownTimer targetDate={targetDate} /></div>);
    }
    return (<div className={styles.placeholder}><h3>No Match Currently In Progress</h3><p>When a match is live, this tab will activate to allow real-time event logging.</p></div>);
}
